...
#[allow(dead_code)]
fn length_of_shortest_hamiltonian_path(adj: Vec<Vec<u32>>) -> u32 {
    let n: usize = adj.len();
            ...
            for neighbor in (0..n).filter(|neighbor| {
                ...
                if dp[subset ^ (1 << node)][neighbor].is_some() {
                    ...
                    if let Some(d) = dp[subset][node] {
                        dp[subset][node] = Some(std::cmp::min(
                            d,
    ...
    // Retrieve smallest path length from subset containing all nodes
    dp[(1 << n) - 1]
        .iter()
...
fn build_adjacency_matrix(locations: &str) -> Vec<Vec<u32>> {
    ...
    for line in locations.lines() {
        ...
        if locations_index.get(location1).is_none() {
        ...
        if locations_index.get(location2).is_none() {
...
mod solution {
    ...
    fn get_length_of_shortest_hamiltonian_path() {
        ...
        assert_eq!(length_of_shortest_hamiltonian_path(adj), 141);
    }

    #[test]
    fn test_length_of_shortest_hamiltonian_path_simple() {
        let adj = vec![vec![0, 464, 518], vec![464, 0, 141], vec![518, 141, 0]];
        assert_eq!(length_of_shortest_hamiltonian_path(adj), 605);
    }
}
